...
Дополнительно реализуем бонусную фичу: отмена незавершенных futures, если мы уже набрали нужное число ответов 
для GET запросов. Для PUT и DELETE мы не хотим использовать данную логику, так как мы хотим поддерживать на репликах
наиболее свежие состояния.

Посмотрим улучшиться ли производительность: очевидно, что производительность при from=3, ack=3 не станет быстрее, так
как мы только добавили дополнительной логики, которая никак не используется.

Замерим результаты для from=3, ack=1:
```
 
```

Как мы видим решение при ack=1 дало прирост к производительности из-за того, что мы не дожидаемся остальных запросов, 
в особенности это заметно когда мы обращаемся к своей ноде и не дожидаемся ответа от других нод